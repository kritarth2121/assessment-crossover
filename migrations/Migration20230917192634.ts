...
export class Migration20230917192634 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table `article` (`id` int unsigned not null auto_increment primary key, `slug` varchar(255) not null, `title` varchar(255) not null, `description` varchar(255) not null, `body` varchar(255) not null, `created_at` datetime not null, `updated_at` datetime not null, `tag_list` text not null, `favorites_count` int not null, `locked_by_user_id` int null, `last_activity_time` datetime null) default character set utf8mb4 engine = InnoDB;',
    );
    ...
    this.addSql(
      'create table `article_authors` (`article_id` int unsigned not null, `user_id` int unsigned not null, primary key (`article_id`, `user_id`)) default character set utf8mb4 engine = InnoDB;',
    );
    ...
    this.addSql(
      'create table `user_favorites` (`user_id` int unsigned not null, `article_id` int unsigned not null, primary key (`user_id`, `article_id`)) default character set utf8mb4 engine = InnoDB;',
    );
    ...
    this.addSql(
      'create table `user_to_follower` (`follower` int unsigned not null, `following` int unsigned not null, primary key (`follower`, `following`)) default character set utf8mb4 engine = InnoDB;',
    );
